...
```bash
docker pull sameersbn/nextcloud:12.0.3
```
...
docker run --name nextcloud -itd --restart=always \
  ...
  sameersbn/nextcloud:12.0.3 app:nextcloud
```
...
docker run --name nextcloud-nginx -itd --restart=always \
  ...
  --publish 10080:80 \
  sameersbn/nextcloud:12.0.3 app:nginx
```
...
docker run --name nextcloud -it --rm [OPTIONS] \
  sameersbn/nextcloud:12.0.3 app:backup:create
```
...
```bash
docker run --name nextcloud -it --rm [OPTIONS] \
  sameersbn/nextcloud:12.0.3 app:backup:restore
```
...
```bash
docker run --name nextcloud -it --rm [OPTIONS] \
  sameersbn/nextcloud:12.0.3 app:backup:restore BACKUP=1417624827_nextcloud_backup.tar
```
